...
- Evidence manifest and widened grounding not merged by **end of Week 8** → the **cross-artifact** query layer is cut. **The decision inspector (PRD §5.11B) is unaffected**: the user can still select any decision, or a timestamp where nothing happened, and ask why. What is lost is asking across many runs in one question.
...
**Cross-artifact, optional (§5.11A).** Asking across fold results, failure clusters, parameter sweeps or another strategy family in a single question. This is gated on **evidence-manifest grounding**: the checker must be generalised from one DecisionRecord to a hashed manifest before any such question is answered, the same order that put `grounding.py` before the explanation service. **Fallback at end of Week 8:** if the manifest and generalised grounding are not done, this layer is cut. The decision inspector is unaffected, so the product does not lose its interaction, only its reach.
