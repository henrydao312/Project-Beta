...
|---|---|---|
| **Guaranteed core = the MVP feature set** | Data pipeline, feature engineering, three selectable rule-based strategy families (§5.3), regime classifier (M1), signal-quality model (M2), risk engine **incl. halt control (§8A)**, execution simulator, paper trading, evaluation harness, explanation agent, failure-analysis assistant, dashboard **incl. replay mode (§19.5)**, **decision inspector (PRD §5.11B)** | Committed - this alone is a complete capstone |
...
| **Selectable strategy families** | **User picks momentum breakout, MA trend or mean reversion at setup (§5.3).** All three run the full pipeline: backtest, AI filters, risk controls, decision logging, grounded explanations, failure analysis and the workbench. **Claim status is per strategy.** The course deliverable implements all three and validates one; MA trend and mean reversion are planned follow-on validation candidates after the course, queued one at a time under the same protocol unless schedule allows one to be pulled forward | Scope change 2026-09-06, `DECISIONS.md` #23 |
| **Protected workbench layer** | **Evidence manifest + cross-artifact query layer (§22.1, PRD §5.11A)**: one question spanning fold results, failure clusters, precomputed sweeps and another family's evidence. **The broader half only**; per-decision interrogation is in the core above. Since evidence interrogation is the USP, extra markets are cut before this layer; if pressured, it narrows before it disappears | Weeks 6-10, gated on manifest grounding by end of Week 8 |
...
- If Week 8 scope pressure threatens the workbench, cut **crypto first, then options**, before cutting the cross-artifact layer. If full taxonomy support is too large, narrow §5.11A to a minimal fixed-question surface over precomputed artifacts. If manifest grounding itself is not merged, then the cross-artifact layer is cut. **The decision inspector (PRD §5.11B) is unaffected**: the user can still select any decision, or a timestamp where nothing happened, and ask why.
...
`generate_candidates(features, bars) -> list[CandidateTrade]` from a transparent rule set. **Three selectable families, chosen at setup** via `strategy.name` in RunConfig: **momentum breakout**, **moving-average trend**, **mean reversion**. Parameters live in RunConfig, not code, so a family's thresholds are part of the run's provenance.

**All three run the entire pipeline** (scope change 2026-09-06, `DECISIONS.md` #23): backtest, the AI filters, the deterministic risk controls and halt, decision logging, grounded explanations, failure analysis, and the evidence workbench. They are selectable paths, not decorative toggles. Previously the secondary families were backtest-only; that is what changed.

**What is gated is the claim, not the path.** `strategy.tier` is a per-strategy claim status. `primary` means the family has been through the full pre-registered protocol - walk-forward at the locked fold scheme, cost stress, leakage checks, ablation, failure analysis - and only such a family may carry a performance claim. `secondary` means it runs everything and reports its own behaviour but carries **no** headline metric, no ablation figure and no Sharpe. A family moves between them **only** by passing the protocol in full, pre-registered before the run. The plan is staged rather than vague: the course funds one protocol run; the other two families remain named follow-on validation candidates after the course, queued one at a time under the same protocol unless schedule allows one to be pulled forward.

**Status today, stated rather than implied:** momentum breakout is the only family that has run the protocol, and its first graded run returned `no_claim`. **No family is currently validated.** `primary` is a statement about which evidence exists, not about whether the rule works.
...
| 7–8 | Signal-quality intelligence | Candidate-trade dataset, quality model (features per the §5.2 verdict), calibration, M2 ablation; **evidence manifest + widened grounding for the query layer**. **Fallback checkpoint: protect a minimal cross-artifact surface; cut extra markets first if scope is tight** | - |
...
| **Selectable strategies dilute evaluation rigor, or become three attempts at one result** | Claim status is per strategy and only a family that has passed the full pre-registered protocol carries a performance figure; claims are strategy-scoped and separately pre-registered; no results table places two families in one comparison; the workbench refuses to rank them. Harness-enforced, not documented |
...
3. **Strategy Engine** checks the fixed rules of the family the user selected at setup - momentum breakout, MA trend or mean reversion - and proposes a candidate trade. No AI. The choice changes the rule, not the pipeline: every family goes through every stage below.
...
**Decision inspector:** click any decision, or any timestamp where nothing happened, and ask why. Sizing, rejection, a risk-engine reduction, or the rule condition that failed when no candidate was generated. Answers come from that decision's own stored fields and are checked against them before display.

**Strategy panel:** the family selected at setup, momentum breakout, MA trend or mean reversion, each running the full pipeline. **The panel shows the family's claim status beside its name**, so a family with no protocol run behind it cannot be read as endorsed, and it offers no ranking.
...
| **3** | **Misleading performance claims.** Leakage, overfitting, an under-modelled cost assumption, an unstated feed limitation, **or a secondary track's result read as if it carried the core's evidentiary weight**. **New with selectable strategies: three families each hunting a positive result is three chances at one finding, and a user free to switch families is free to keep going until one looks good** | Walk-forward only, point-in-time features and instrument universe (§11), cached AI outputs, prespecified primary metric fixed before M1 experiments, negative results framed as legitimate (§18). **Feed, asset class, fold count and strategy recorded wherever results appear (§7A, §5.3)**. **Claims are strategy-scoped and separately pre-registered; ranking families is not a supported claim, and a family carries a performance figure only at `tier: primary`** | Leakage test (shift-forward invariance); backtest cache-only enforcement; 2–3× cost-stress runs reported *alongside* headline numbers; **schema test asserting every results row carries `asset_class`, `n_folds`, `strategy` and `strategy_version`**; the harness refuses a graded ablation for a secondary family and refuses to place two families in one comparison table |
| **4** | **A conversational surface that answers past its evidence, or is steered into advising** (new with §5.11A). Three modes: **injection through user text**; an answer composing many artifacts that drifts beyond what any of them support; an answer phrased as a recommendation. *Today injection is impossible **by construction**, because every prompt is assembled from record fields and no text the system did not author reaches a model. The query layer ends that property, so it must be replaced by a defence* | Read-only and evidence-bounded: retrieval only from the hashed manifest, **no tools, no market access, no write path, no open-ended chat**. User text is carried in a delimited slot the system prompt declares to be data, never instruction. **Refusal path for trade recommendations, proposed strategy changes, and requests to rank or choose between strategy families, as a code path rather than a prompt instruction.** The user selects the strategy; the system reports evidence for it and never endorses one. Every claim cites an artifact, record or sweep-cell id | **Adversarial query corpus**: every item refused or answered from evidence only, none causing an instruction in user text to be executed. Manifest grounding: every numeral and state label traces to a cited artifact. **Recommendation-request suite asserts refusal**, including requests phrased as evidence questions. An answer carrying an uncited claim fails. **A rubric-assisted LLM judge, if added for soft prose claims, may only downgrade**: a test asserts a claim failing the deterministic check stays blocked whatever the judge returns |
...
**Role of AI: background automation, plus non-advisory analysis.** The AI runs unattended inside the closed loop, where it classifies the market, scores candidate trades and afterwards writes explanations, and **it never converses with the user to reach a trade decision**. Separately, and outside the loop, the user can ask bounded questions about one recorded decision or non-decision: why it was sized that way, why it was rejected, why risk reduced it, or why nothing fired at a timestamp. The optional cross-artifact layer widens that reach to fold results, failure clusters, sweeps and another strategy family's evidence. Both surfaces are read-only, evidence-bounded and non-advisory: they explain logged or computed evidence, and they refuse to recommend a trade, propose a change to trading logic, or tell the user which strategy family to pick (§5.11A, §5.11B). **The user chooses the strategy; the system supplies the evidence and its status, never the endorsement.**

**Two surfaces, one in the core and one gated.** The distinction matters because the cut line only touches one of them.

**Decision-level, guaranteed core (PRD §5.11B).** The user selects a decision, or a timestamp where no decision happened, and asks a bounded question about that one thing: why was this trade sized this way, why was this signal rejected, why did the risk engine reduce the position, why did no trade occur here. The answer is assembled from that decision's own stored fields - the DecisionRecord, the candidate-trade record, the risk state, the deterministic rule state and the reason codes - and nothing else. **It needs no manifest**, because `grounding.py` already checks a text against a single record and is already tested. The one genuinely new piece is a **rule-state trace** so a non-event can be explained: where the trace exists the answer names the failed rule condition, and where it does not the system says so rather than inferring a reason.

**Cross-artifact, protected but gated (§5.11A).** Asking across fold results, failure clusters, parameter sweeps or another strategy family in a single question. This is gated on **evidence-manifest grounding**: the checker must be generalised from one DecisionRecord to a hashed manifest before any such question is answered, the same order that put `grounding.py` before the explanation service. **Fallback at end of Week 8:** because this is part of the USP, crypto and options are cut before it. If full taxonomy support is too large, this layer narrows to a minimal fixed-question surface over precomputed artifacts. If manifest grounding itself is not done, this layer is cut. The decision inspector is unaffected either way.
...
| 5 | Answer bounded questions about one decision or non-decision | **Text generation** over one stored record, checked by the existing grounding filter | **Buy** the model; **build** the decision inspector and rule-state trace |
| 6 | Answer bounded questions over the evidence manifest | **Retrieval + text generation** over a closed, hashed, system-authored corpus | **Buy** the model; **build** the manifest, retrieval boundary and verifier; optional behind the Week 8 gate |
